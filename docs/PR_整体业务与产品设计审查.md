...
### 1.7 报告下发、归档与申诉（文档 02 / 00 补充约定）

本节为发证后至归档的完整流程与商业规则，与 02 任务调度与状态机、00 全局数据字典一致。
...
| 任务状态 | CONFIG→COLLECT→AUDIT→CALC(LCA)→VERIFY→CERTIFIED；发证后 下发→待归档→已归档 | task_list 0–6：配置/采集/计算/核查/**下发**/待归档/已归档；CALC 即 LCA（独立系统、手工摆渡、预留接口） | 已对齐 |
...
| 报告下发与申诉 | 下发前上传用印版核算报告（版本替换）；复选框选择报告/证书下发；下发后、确认接收前可申诉；确认接收后进入待归档/归档，无申诉操作 | 任务列表已增加「下发」阶段；报告管理页无「下发前用印版上传」与「复选框下发」；待归档/已归档不展示申诉按钮；供应商端已有「确认接收」与申诉窗口 | 部分对齐 |
...
- **建议**：02/00/供应商工作台文档按 1.7 与计划修订（2.5 下发前用印版与版本、2.6 申诉与确认接收、任务阶段与下发对应）；任务列表已增加「下发」阶段（0–6：配置/采集/计算/核查/下发/待归档/已归档）；运营端 report_mgt/report_detail 对待归档与已归档不展示申诉处理按钮；供应商端 reports 已增加「确认接收」与「仅未确认接收时可申诉」；报告管理页后续迭代支持「下发前上传用印版」与「复选框下发」。
...
| 高 | 报告下发、确认接收与申诉 | 02/00/供应商文档更新（下发前用印版、复选框下发、确认接收、申诉窗口、待归档/已归档无申诉按钮）；运营端待归档/已归档不展示申诉按钮；供应商端确认接收与申诉窗口 |
...
| 低 | 报告管理下发与版本 | 报告管理页支持下发前上传用印版核算报告（版本记录）、复选框勾选报告/证书下发（后续迭代） |
...
- **文档**：`docs/00_全局数据字典与枚举.md`、`docs/01_订单管理逻辑.md`、`docs/02_任务调度与状态机.md`、`docs/03_模板引擎解析逻辑.md`、`docs/供应商工作台功能清单与信息结构.md`  
...
本 PR 为**产品设计层面审查结论**，不包含代码改动；可按优先级拆分为具体需求或开发任务（如：更新 00/02 文案、门户开放入口、订单页加状态字段、报告下发与申诉规则在 02/00/供应商文档中落地、运营端待归档/已归档不展示申诉按钮、供应商端确认接收与申诉窗口等）单独落地。
