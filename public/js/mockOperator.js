...
(function (global) {
  ...
  var MOCK_REPORTS = [
    ...
    },
    {
      ...
      received: false,
      docs: {
        calc: { name: '超高功率石墨电极核算报告_用印版.pdf', ver: 'V1', date: '2026-02-08' },
        verify: { name: '核查报告_TUV.pdf', ver: 'V1', date: '2026-02-08' },
    ...
    },
    {
      ...
      received: false,
      appeal: {
        reason: '核算边界与产品规格说明不一致，申请重新核查。',
        submitTime: '2026-02-09 14:30',
      },
      docs: {
  ...
  function getSupplierReports(supplierName) {
    ...
    return MOCK_REPORTS.filter(function (r) {
      ...
      var statusText =
        r.status === 'archived' ? '已发证' : r.releasedToSupplier ? '已下发' : '待处理';
      var reportDate =
        r.docs && r.docs.calc && r.docs.calc.date
          ? r.docs.calc.date
          : (r.archiveTime || '').slice(0, 10);
      return {
        taskNo: r.taskNo,
        taskId: r.taskNo,
        orderNo: r.orderNo,
   ...
   */
  function submitAppeal(taskNo, reason, _attachments) {
    for (var i = 0; i < MOCK_REPORTS.length; i++) {
      if (MOCK_REPORTS[i].taskNo === taskNo) {
  ...
  var GOLDEN_DEMO_TASK_NO = 'TSK-2026-888';
  var GOLDEN_DEMO_REPORT_ID = 'RPT-2026-0001';

  function resetGoldenDemoReportState() {
    for (var i = 0; i < MOCK_REPORTS.length; i++) {
      if (MOCK_REPORTS[i].taskNo !== GOLDEN_DEMO_TASK_NO) continue;
      MOCK_REPORTS[i].reportId = GOLDEN_DEMO_REPORT_ID;
      MOCK_REPORTS[i].status = 'process';
      MOCK_REPORTS[i].reportVersion = 'sealed';
      MOCK_REPORTS[i].releasedToSupplier = false;
      MOCK_REPORTS[i].received = false;
      delete MOCK_REPORTS[i].appeal;
      delete MOCK_REPORTS[i].archiveTime;
      MOCK_REPORTS[i].docs = {
        calc: { name: '超高功率石墨电极核算报告_用印版.pdf', ver: 'V1', date: '2026-02-08' },
        verify: { name: '核查报告_TUV.pdf', ver: 'V1', date: '2026-02-08' },
        cert: { name: '声明证书.jpg', ver: 'V1', expire: '2027-02-08' },
      };
      MOCK_REPORTS[i].history = [
        {
          title: '核查通过',
          desc: '任务进入报告阶段，待运营下发',
          timestamp: '2026-02-08 16:00',
  ...
  /** 黄金动线：同时重置任务与报告 Mock（需已加载 mockTasks.js） */
  function resetGoldenDemoState() {
    if (typeof global.resetGoldenDemoTaskState === 'function') global.resetGoldenDemoTaskState();
    resetGoldenDemoReportState();
  }
  ...
  global.resetGoldenDemoReportState = resetGoldenDemoReportState;
  global.resetGoldenDemoState = resetGoldenDemoState;
