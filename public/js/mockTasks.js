...
(function (global) {
  ...
  function getTaskInfoFromUrl(defaultTaskId) {
    ...
    if (defaultTaskId && MOCK_TASK_MAP[defaultTaskId])
      return Object.assign({}, MOCK_TASK_MAP[defaultTaskId]);
  ...
  (function seedSupplierDemo() {
    for (var id in MOCK_TASK_MAP) {
      ...
      if (id === 'TSK-2026-899')
        continue; /* 配置中演示：无 snapshot，供应商列表可见，点进展示 task_stage_config 配置中页 */
  ...
  var TASK_CREATE_TIMES = {
    'TSK-2026-888': '2026-02-03 09:00',
    'TSK-2026-889': '2026-02-03 09:30',
    'TSK-2026-890': '2026-02-01 14:20',
    'TSK-2026-891': '2026-02-04 10:00',
    'TSK-2026-893': '2026-02-10 11:00',
    'TSK-2026-894': '2026-02-11 08:30',
    'TSK-2026-895': '2026-02-15 14:00',
    'TSK-2026-896': '2026-02-16 09:00',
    'TSK-2026-897': '2026-02-18 10:30',
    'TSK-2026-898': '2026-02-18 15:00',
    'TSK-2026-899': '2026-02-19 10:00',
  };
  var TASK_DEADLINES = {
    'TSK-2026-888': '2026-02-20',
    'TSK-2026-889': '2026-02-18',
    'TSK-2026-890': '2026-02-15',
    'TSK-2026-891': '2026-02-22',
    'TSK-2026-893': '2026-02-25',
    'TSK-2026-894': '2026-02-28',
    'TSK-2026-895': '2026-03-02',
    'TSK-2026-896': '2026-02-01',
    'TSK-2026-897': '2026-03-05',
    'TSK-2026-898': '2026-03-01',
    'TSK-2026-899': '2026-03-08',
  };
  function getTaskCreateTime(taskId) {
    return TASK_CREATE_TIMES[taskId] || '2026-02-01 09:00';
  }
  function getTaskDeadline(taskId) {
    return TASK_DEADLINES[taskId] || '';
  }
  ...
  var SUPPLIER_TASK_CURRENT_STAGE = {
    'TSK-2026-888': { stageIndex: 0, stageSubStatus: 'pending' },
  ...
  function getSupplierTaskStageRow(taskId, supplierName) {
    if (SUPPLIER_TASK_CURRENT_STAGE[taskId])
      return Object.assign({}, SUPPLIER_TASK_CURRENT_STAGE[taskId]);
    if (typeof MOCK_TASK_LIST !== 'undefined') {
      for (var i = 0; i < MOCK_TASK_LIST.length; i++) {
        if (MOCK_TASK_LIST[i].taskId === taskId && MOCK_TASK_LIST[i].supplier === supplierName)
          return {
            stageIndex: MOCK_TASK_LIST[i].stageIndex,
            stageSubStatus: MOCK_TASK_LIST[i].stageSubStatus,
            reportSubStatus: MOCK_TASK_LIST[i].reportSubStatus,
          };
      }
    }
  ...
  function getSupplierTaskList(supplierName) {
    ...
    for (var taskId in MOCK_TASK_MAP) {
      ...
      var clarifications =
        typeof getClarificationsByTaskId === 'function'
          ? getClarificationsByTaskId(taskId, 'supplier')
          : [];
      var openClarify = null;
      for (var i = 0; i < clarifications.length; i++) {
        if (
          clarifications[i].status === 'open' &&
          (clarifications[i].initiator === 'operator' || clarifications[i].initiator === 'verifier')
        ) {
      ...
      if (stageIndex === 0) {
        ...
        if (
          openClarify &&
          (stageSubStatus === 'clarifying' || stageSubStatus === 'waiting_supplier')
        )
          taskType = 'clarification';
        else taskType = '';
      ...
      var stageStatusText =
        typeof getStageStatusText === 'function'
          ? getStageStatusText(stageRow)
          : getSupplierStageText(taskType, taskId);
  ...
  /**
   * 核查端待核查任务列表：从全局任务列表中筛选阶段为「核查认证」(stageIndex === 3) 的任务，并补全 specs。
   * 与 getTaskList/getSupplierTaskList 共用数据源，保证三端流程可串联。
   * @returns {Array<{taskId, orderNo, productName, specs, supplier, stageIndex, stageSubStatus, deadline, isOverdue, ...}>}
   */
  function getCertifierTaskList() {
    var full = getTaskList();
    var out = [];
    for (var i = 0; i < full.length; i++) {
      if (full[i].stageIndex !== 3) continue;
      var row = Object.assign({}, full[i]);
      if (MOCK_TASK_MAP[row.taskId] && MOCK_TASK_MAP[row.taskId].specs)
        row.specs = MOCK_TASK_MAP[row.taskId].specs;
  ...
  function getTaskByTaskId(taskId, pageStageIndex) {
    ...
    var wantStage =
      typeof pageStageIndex === 'number' && pageStageIndex >= 0 && pageStageIndex <= 4;
  ...
  /** 五段任务阶段中文名（0 配置 → 4 报告），与 docs/04_任务调度与状态机 一致 */
  var STAGE_LABELS = ['配置', '采集', '计算', '核查', '报告'];
  ...
  function getStageLabel(stageIndex) {
    if (typeof stageIndex !== 'number' || stageIndex < 0 || stageIndex > 4) return '—';
  ...
  global.getCertifierTaskList = getCertifierTaskList;
  global.getTaskByTaskId = getTaskByTaskId;
  global.STAGE_LABELS = STAGE_LABELS;
  global.getStageLabel = getStageLabel;
  ...
  function getSupplierOrderList(supplierName) {
    var taskList =
      typeof getSupplierTaskList === 'function' ? getSupplierTaskList(supplierName) : [];
    var byOrder = {};
  ...
  var GOLDEN_DEMO_TASK_ID = 'TSK-2026-888';

  /** 将黄金动线演示任务 TSK-2026-888 恢复为 stage 0 / 待配置模板（内存 Mock） */
  function resetGoldenDemoTaskState() {
    SUPPLIER_TASK_CURRENT_STAGE[GOLDEN_DEMO_TASK_ID] = { stageIndex: 0, stageSubStatus: 'pending' };
    for (var i = 0; i < MOCK_TASK_LIST.length; i++) {
      if (MOCK_TASK_LIST[i].taskId !== GOLDEN_DEMO_TASK_ID) continue;
      MOCK_TASK_LIST[i].stageIndex = 0;
      MOCK_TASK_LIST[i].stageSubStatus = 'pending';
      MOCK_TASK_LIST[i].deadline = '2026-02-10';
      MOCK_TASK_LIST[i].isOverdue = false;
      delete MOCK_TASK_LIST[i].reportSubStatus;
      delete MOCK_TASK_LIST[i].hasAppeal;
    }
    if (typeof setTaskRejectReason === 'function') setTaskRejectReason(GOLDEN_DEMO_TASK_ID, '');
  }

  global.resetGoldenDemoTaskState = resetGoldenDemoTaskState;
