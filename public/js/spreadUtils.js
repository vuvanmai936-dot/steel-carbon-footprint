...
(function (global) {
  ...
  var _licenseApplied = false;

  /**
   * 注入 SpreadJS License（正式环境由服务端注入 window.PCF_SPREADJS_LICENSE 或 meta 标签）
   */
  function applySpreadLicense(licenseKey) {
    if (_licenseApplied) return true;
    var key =
      licenseKey ||
      (typeof global.PCF_SPREADJS_LICENSE === 'string' ? global.PCF_SPREADJS_LICENSE : '') ||
      '';
    if (!key) return false;
    try {
      if (GC.Spread && GC.Spread.Sheets && typeof GC.Spread.Sheets.LicenseKey !== 'undefined') {
        GC.Spread.Sheets.LicenseKey = key;
        _licenseApplied = true;
        return true;
      }
    } catch (e) {
      console.warn('spreadUtils.applySpreadLicense:', e);
    }
    return false;
  }

  /**
   * 统一创建 Workbook（自动 apply license、culture）
   * @param {HTMLElement} hostEl
   * @param {Object} [workbookOptions] 传给 Workbook 构造器的第二参数
   */
  function createWorkbook(hostEl, workbookOptions) {
    applySpreadLicense();
    ...
    return new SpreadNS.Workbook(hostEl, workbookOptions);
  }
  ...
  function createSpreadFromSnapshot(hostEl, snapshot, options) {
    ...
    const spread = createWorkbook(hostEl, { sheetCount: sheetNames.length });
  ...
  function getSnapshotFromSpread(spread, extra) {
    ...
    for (let i = 0; i < spread.getSheetCount(); i++) {
      ...
      if (usedRange) {
        ...
        sheetData[name] =
          arr.length > 0 ? arr : [['类别', '名称', '单位', '填报指引', '数值', '备注']];
      } else {
  ...
  function parseExcelFile(file) {
    if (
      !file ||
      typeof (global.XLSX || (typeof window !== 'undefined' && window.XLSX)) === 'undefined'
    ) {
