...
(function (global) {
  ...
  /** 运营端任务详情 el-header 统一样式（见 docs/03_任务详情/12） */
  var TASK_DETAIL_HEADER_STYLE =
    'background: #fff; border-bottom: 1px solid #dcdfe6; height: 60px; display: flex; justify-content: space-between; align-items: center; padding: 0 20px; flex-shrink: 0;';

  function formatSlaSeconds(totalSec) {
    var sec = Math.max(0, Math.floor(totalSec || 0));
  ...
  /** 为任务详情 Vue 选项注入 SLA 倒计时（Options API） */
  function applyTaskDetailSlaOptions(appOptions, initialSeconds) {
    initialSeconds = initialSeconds == null ? 72 * 3600 : initialSeconds;
    var origData = appOptions.data;
    appOptions.data = function () {
      var d = typeof origData === 'function' ? origData.call(this) : {};
      d.slaRemaining = d.slaRemaining != null ? d.slaRemaining : initialSeconds;
      d._slaTimerId = null;
      return d;
    };
    var origMethods = appOptions.methods || {};
    appOptions.methods = Object.assign({}, origMethods, {
      formatSla: function (sec) {
        return formatSlaSeconds(sec != null ? sec : this.slaRemaining);
      },
      startSlaTimer: function () {
        var self = this;
        if (self._slaTimerId) return;
        self._slaTimerId = setInterval(function () {
          if (self.slaRemaining > 0) self.slaRemaining--;
        }, 1000);
      },
    ...
    var origMounted = appOptions.mounted;
    var origUnmounted = appOptions.unmounted;
    appOptions.mounted = function () {
      this.startSlaTimer();
      if (typeof origMounted === 'function') origMounted.call(this);
    };
    appOptions.unmounted = function () {
      this.stopSlaTimer();
      if (typeof origUnmounted === 'function') origUnmounted.call(this);
    };
    return appOptions;
  }
  ...
  var api = {
    ...
    TASK_DETAIL_HEADER_STYLE: TASK_DETAIL_HEADER_STYLE,
    formatSlaSeconds: formatSlaSeconds,
    applyTaskDetailSlaOptions: applyTaskDetailSlaOptions,
