...
> **外发约束（2026-05-09）**：本目录中的报价、工作量评估、排期说明等材料**默认视为对内材料**，外发前必须按"客户名 / 单价 / 内部里程碑 / 团队人名 / 内部 GitLab 路径"等敏感字段脱敏。仓库根目录 `.gitattributes` 已将本目录标记为 `export-ignore`，`git archive` 打包默认不包含本目录；如需外发，建议**按客户单独打包并人工二次审阅**。后续若改造为独立 Git 子库，请在 [`docs/00_迭代计划/设计变更记录.md`](../docs/00_迭代计划/设计变更记录.md) 登记一行。
